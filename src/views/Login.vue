<template>
    <div class="container">
       ...
       <form @submit="get_data" method="post">
        ...
        <input class="input" type="text" placeholder="Username or email" v-model="this.users_data.usernameOremail" required id="username" autocomplete="name" name="username">
        <input class="input" type="password" name="password" id="password" v-model="this.users_data.password" placeholder="Password" autocomplete="current-password">
      ...
      axios
        ...
        .then((Response) => {
          ...
          this.response_data.some(e => {
            ...
            if((e.user_name==this.users_data.usernameOremail
            ...
            if(flag2 && counter==this.response_data.length){//checks the user found or not and check all users checked or not
            
                document.querySelector(".alert_box").classList.add("alert_animation")
                setTimeout(() => {// remove the animation class after 5 second
              document.querySelector(".alert_box").classList.remove("alert_animation");
        }, 5000);
                this.flag=false
            }
            counter++;
...
@keyframes myanimation {
    ...
    30%{top: 3em;opacity: 1;}
    80%{top: 3em;opacity: 1;}
